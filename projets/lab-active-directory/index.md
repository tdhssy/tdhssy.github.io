...
**IIS en reverse proxy plutôt qu'un hébergement Python natif.** La documentation Microsoft
...
**Une application dynamique plutôt qu'une page de démonstration.** Je voulais voir concrètement
comment faire interagir Python avec le système Windows : l'application « IT » interroge WSUS en
direct via PowerShell lancé en sous-processus, et affiche le statut des postes enregistrés.
...
premier, ce qui m'aurait évité une analyse aussi poussée. La leçon n'est pas sur la syntaxe :
c'est de vérifier le trivial avant le complexe.
...
**Et le passage en mode DSRM**, lors du test de restauration, a révélé deux dépendances qu'on ne
voit pas en fonctionnement normal. Le DNS intégré à AD DS étant hors ligne dans ce mode, la
...
- Sauvegarde de l'état système d'Alpha-win planifiée vers un partage sur Beta-win, **et test
  réel de restauration en mode DSRM**. Une sauvegarde jamais restaurée n'est pas une sauvegarde.
